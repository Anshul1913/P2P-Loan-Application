...
const StudentRegisterModal = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await AuthApi.signupBorrower(loginDTO);
  ...
  return (
    <ModalWrapper title="Student Registration" emoji="🎓" onClose={onClose}>
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="bg-emerald-500 hover:bg-emerald-600 w-full text-white font-semibold py-2 rounded-md">
          Start My Journey
        </button>
      </form>
    </ModalWrapper>
